...
model: opus
tools: ["Agent", "Read", "Write", "Glob", "Grep", "Bash", "mcp__e6baa7ac-3009-414a-8204-97d82293cc6c__notion-create-pages", "mcp__e6baa7ac-3009-414a-8204-97d82293cc6c__notion-search", "mcp__e6baa7ac-3009-414a-8204-97d82293cc6c__notion-fetch"]
---
...
- **9-10**: Directly applicable technique/tool for an existing project (weather-bot, kalshi-arb, OrgIQ, lego-investor, breathingCode)
- **7-8**: Useful AI/dev technique that could improve current workflows or projects
...
Launch `opportunity-analyst` agent (one per bookmark or batched if >5). Provide the full bookmark object plus the mission context.

### For SKILL bookmarks:
Launch `skill-compiler` agent (can batch multiple skill bookmarks). Provide all SKILL bookmark objects plus the project portfolio.
...
3. **Dispatch to implementation-planner**: Launch the `implementation-planner` agent for each GO opportunity. Provide the full opportunity assessment.
...
Search Notion for an existing database called "Bookmark Income Pipeline". If not found, report that it needs to be created (the command will handle creation on first run).
...
- **Synergy Projects**: which existing projects this relates to
...
For opportunities with plausibility >= 8 AND go_nogo == "GO", include the full implementation plan as the page content with:
- Phase breakdown (Validate → MVP → Launch)
- Revenue projection table
- "Do RIGHT NOW" steps as checkboxes
- Risk mitigations and kill criteria

## Phase 6: Output to Markdown

Write a summary to `/Users/willvowell/twitterBookmarks/output/run-{YYYY-MM-DD}.md`:
...
### {Opportunity Name}
{Brief summary + link to Notion page}
...
- "Full results in Notion: Bookmark Income Pipeline"
